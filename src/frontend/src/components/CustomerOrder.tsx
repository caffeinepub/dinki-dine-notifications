...
export function CustomerOrder({ mode = "customer" }: CustomerOrderProps) {
  ...
  const validateForm = (): boolean => {
    if (mode === "drivein") {
      ...
      if (!tableNumber.trim()) {
        setFormError("Please select your table number to continue.");
        return false;
      }
    }
    setFormError("");
      ...
      <div className="max-w-lg mx-auto px-4 h-14 flex items-center gap-3">
        {showBack && (
          <button
            ...
            className="w-9 h-9 rounded-full bg-orange-50 flex items-center justify-center text-orange-500 hover:bg-orange-100 transition-colors flex-shrink-0"
          >
        ...
        <div className="min-w-0">
          ...
          <p className="text-xs text-gray-500 truncate">
            {mode === "drivein" ? "Drive-In Self Order" : "Dine-In & Takeaway"}
          </p>
        </div>
        {screen === "menu" && (
          <div className="ml-auto">
            <span className="text-xs text-gray-400 font-medium">
              {mode === "drivein" ? `🚗 ${carPlate}` : `🍽️ Table ${tableNumber}`}
            </span>
          </div>
        )}
      </div>
        ...
        <main className="flex-1 max-w-lg mx-auto w-full px-4 py-6">
          ...
          <div className="flex items-center gap-2 mb-6">
            <div className="flex items-center gap-1.5">
              <div className="w-6 h-6 rounded-full bg-cust-primary text-white text-xs font-bold flex items-center justify-center">
                1
              </div>
          ...
          <div
            ...
            className={`bg-white rounded-2xl shadow-md border-2 p-5 ${
              mode === "drivein" ? "border-amber-200" : "border-orange-200"
            }`}
          >
            {mode === "drivein" ? (
              <>
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500 mt-0.5">
                      Enter your car details to start ordering
                    </p>
                  </div>
                </div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full h-11 px-3 rounded-xl border-2 border-gray-200 focus:border-amber-400 focus:outline-none text-gray-800 text-sm transition-colors"
                      />
                    </div>
                    <div>
                      <label
          ...
          <button
            ...
            onClick={handleNext}
            className="mt-5 w-full h-13 flex items-center justify-center gap-2 bg-cust-primary hover:bg-cust-primary-dark text-white font-bold rounded-2xl h-14 shadow-lg shadow-orange-200 transition-colors"
          >
            <span>View Menu &amp; Order</span>
            <ArrowRight className="w-4 h-4" />
          </button>

          <p className="mt-6 text-xs text-cust-muted text-center">
  ...
  return (
    ...
    >
      ...
      <main className="flex-1 max-w-lg mx-auto w-full px-4 py-4 pb-36">
        {/* Confirmation banner showing selected table/car */}
        <div
        ...
        >
          ...
          <div className="min-w-0">
            <p className="text-xs text-gray-500">
              {mode === "drivein" ? "Car Number" : "Your Table"}
            </p>
            <p className="text-sm font-bold text-gray-800 truncate">
              {mode === "drivein" ? carPlate : tableNumber}
              {mode === "drivein" && carMake && (
          ...
          <button
            type="button"
            onClick={() => setScreen("form")}
            className="ml-auto text-xs text-gray-400 underline hover:text-gray-600 flex-shrink-0"
          >
            Change
          </button>
        </div>
